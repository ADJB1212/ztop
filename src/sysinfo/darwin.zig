...
fn mapTcpState(state: c_int) common.NetConnState {
    return switch (state) {
...
test "parseSocketFdInfo extracts IPv4 TCP endpoints" {
    ...
    const conn = parseSocketFdInfo(42, process_name, 4, &socket_info).?;
...
test "parseSocketFdInfo extracts IPv6 UDP endpoints" {
    ...
    const conn = parseSocketFdInfo(77, process_name, 3, &socket_info).?;
...
test "parseSocketFdInfo rejects unsupported socket kind" {
    ...
    try std.testing.expectEqual(@as(?common.NetConnection, null), parseSocketFdInfo(9, process_name, 4, &socket_info));
}

test "mapTcpState covers expected transitions" {
    try std.testing.expectEqual(common.NetConnState.listen, mapTcpState(c.TSI_S_LISTEN));
    try std.testing.expectEqual(common.NetConnState.close_wait, mapTcpState(c.TSI_S__CLOSE_WAIT));
    try std.testing.expectEqual(common.NetConnState.time_wait, mapTcpState(c.TSI_S_TIME_WAIT));
    try std.testing.expectEqual(common.NetConnState.unknown, mapTcpState(999));
}
